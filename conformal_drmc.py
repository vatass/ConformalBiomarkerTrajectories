...
def predict_and_update_results_per_subject(
    ...
    population_results, population_mae_kfold, population_metrics_per_subject,z, gpuid=-1
):
    device = torch.device(f"cuda:{gpuid}" if gpuid >= 0 and torch.cuda.is_available() else "cpu")
    model = model.to(device)
...
for fold in range(10): 
    ...
    max_train_residual = np.max(np.abs(np.array(train_results['score']) - np.array(train_results['y'])))
    ...
    # Calculate the MTV Quantile
    conformal_scores_mtv = np.array(conformity_scores_per_subject['conformal_scores_mtv'])
    ...
    print('Test the Conformalized Deep Regressor with the MTR Non-Conformity Scores')
    test_time = np.array(test_x[:, -1].cpu().detach().numpy())  # Convert directly to NumPy array
    test_ids = corresponding_test_ids
    # convert test_ids to str
    test_ids = [str(i) for i in test_ids]
    ...
    # Perform inference and update results
    conformal_mtv_results = conformalized_inference_with_train_sigma(
        model=conformal_model,
        test_x=test_x,
        test_y=test_y,
        test_ids=test_ids,
    ...
    for study in external_studies:
        ...
        external_conf_results = conformalized_inference(
            model=conformal_model,
            test_x=test_x,
            test_y=test_y,
            test_ids=test_ids,
        ...
        # Test and update results with the MTV
        external_conf_mtv_results = conformalized_inference_with_train_sigma(
            model=conformal_model,
            test_x=test_x,
            test_y=test_y,
            test_ids=test_ids,
            ...
            num_samples=100,
            kfold=fold,
            results=external_conf_mtv_results,
            gpuid=gpuid, 
            qhat=qhat_mtv, 
            sigma=max_train_residual,
            study=study
...
qhat_df.to_csv("./conformalresults/qhat_drmc_"+str(expID)+".csv", index=False)
population_results_df.to_csv("./conformalresults/deep_regression_mc_results_"+str(expID)+".csv", index=False)
conformal_results_df.to_csv("./conformalresults/deep_regression_mc_conformal_results_"+str(expID)+".csv", index=False)
conformal_mtv_results_df.to_csv("./conformalresults/deep_regression_mc_conformal_mtv_results_"+str(expID)+".csv", index=False)
external_results_df.to_csv("./conformalresults/deep_regression_mc_external_results_"+str(expID)+".csv", index=False)
external_conf_results_df.to_csv("./conformalresults/deep_regression_mc_external_conf_results_"+str(expID)+".csv", index=False)
external_conf_mtv_results_df.to_csv("./conformalresults/deep_regression_mc_external_mtv_conf_results_"+str(expID)+".csv", index=False)
population_mae_kfold_df.to_csv("./conformalresults/deep_regression_mc_mae_kfold_"+str(expID)+".csv", index=False)
population_metrics_per_subject_df.to_csv("./conformalresults/deep_regression_mc_metrics_per_subject"+str(expID)+".csv", index=False)